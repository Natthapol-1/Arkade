...
- **Panel border is intentionally neutral.** `.crt-frame`'s `border-color` is left at the default `var(--border)` (a near-invisible `#1a1a2e`) at all difficulty levels — difficulty is signaled *only* by the `box-shadow` glow color, matching the glow-only treatment Painting Python and Outcast Assembly use for their boards. Don't reintroduce a `borderColor` override tied to difficulty.
