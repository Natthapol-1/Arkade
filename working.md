...
- **Architecture:**
  ...
  - `engine.ts`: Full `GameState` type, `tick()`, BFS enemy pathfinding, `useLaser` / `activateWave` / `useBullet` / `useSpeedBoost` / `useBomb` / `toggleCarryRuby` / `doTeleport` / `cancelTeleport` exports, meteorite system, boss/Shadow Queen spawn system, resource spawning, electric chain buff.
...
- **Mechanics:**
  ...
  - Every ~30 s a chamber is targeted, cycling clockwise (ALPHA → BETA → DELTA → GAMMA → repeat, tracked by `meteoriteChamberSeq`) rather than randomly (5 s warning on minimap + banner); meteorite wipes all enemies there. Fiery King and Shadow Queen instead lose 75% of their max HP (dying outright if already at or below that).
  - Boss spawns via `bossTimer` / `bossWarningTicks` (~12s after the previous boss-tier enemy dies, since tier 1); 50/50 coin flip between the tank Fiery King and the Shadow Queen — only one of the two can be alive at a time, and the warning banner stays generic either way. Kill = full HP + ruby HP + energy restore for both. A god-mode-only (`?god=1`) panel can also force-spawn either one on demand via `godSpawnBoss`.
