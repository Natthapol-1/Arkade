...
- **Architecture:**
  - `constants.ts`: Tile types, 51×51 map generator, chamber bounds, per-chamber spawn points, 14 enemy configs (12 regular + boss + splitter_queen/queen_echo) with `attackRange` field, all ability & difficulty tuning constants, `QUEEN_*` tuning.
...
- **Mechanics:**
  - Ruby carry vs place: carry slows movement (0.4×) but enemies only target player; placing lets enemies split attention.
  ...
  - Every ~30 s a random chamber is targeted (5 s warning on minimap + banner); meteorite wipes all enemies there. Boss and Splitter Queen just take 75 dmg instead of dying outright.
  - Boss spawns via `bossTimer` / `bossWarningTicks` (~12s after the previous boss-tier enemy dies, since tier 1); 50/50 coin flip between the tank Boss and the Splitter Queen — only one of the two can be alive at a time, and the warning banner stays generic either way. Kill = full HP + ruby HP + energy restore for both.
...
- **Boss-tier (mutually exclusive, one at a time):**
  - **boss** ("Fiery King") — 121 HP base, BFS-chases the player, crimson body (1.55× tile), gold crown w/ ruby-gem twinkle, corner rivets, jagged mouth, push-immune. Fiery ambient FX: rising embers, ground-fire glow, flickering flame-lick spike tips, orange fire-aura outline; laser has a true fire-gradient + dense ember-storm trail + a `spawnFireBurst` impact burst on the player. (Display name only — the `EnemyType` key stays `'boss'`.)
  - **splitter_queen** ("the Queen") — 55 HP base, never chases; fires through walls (no LoS check) at up to 16 tiles within her own chamber only. Phase-jumps to a different chamber every ~8s (never her own or the player's current chamber), leaving a 1-HP `queen_echo` behind. Echoes copy her attack (8 dmg player / 4 dmg ruby, vs. her own 22/18) and **persist forever** — no self-expiry, they stack if ignored, only dissolve on kill (individually, or all at once if the real Queen dies). Regal violet visuals: full gem/tiara/epaulette detail, twinkling gems, a revolving 12-segment magic ring, dual mote orbits, idle aura; laser has an arcane shimmer + wavy ripple + dense twinkling-mote trail. Echo shares the same look but paler/simplified (no mouth, glitch-scanline tell).
- **Key engine patterns:** `damageEnemy(state, e, amount)` — halves if shielder shields target. `killEnemy(state, idx, fromMeteor=false)` — `skipRewards = fromMeteor || type==='queen_echo'`; killing the real Queen also sweeps and dissolves every remaining echo. `chainElectric(state, hitEnemy)` — chains to 2 nearby enemies when `electricBuffTicks > 0`. Boss/Queen use BFS or stay put respectively, both push-immune.
- **Gotchas:** `chamberOfTile` returns -1 for hallways. Wave ticks denominator hardcoded to 30; bomb blast to 28. `doTeleport` destinations hardcoded `[[10,10],[39,10],[10,39],[39,39]]` — must match `TELEPORT_PADS`. `mini_splitter`, `boss`, `splitter_queen`, `queen_echo` are NOT in difficulty tiers (boss-tier uses its own timer; echoes only spawn from a phase-jump). `killEnemy` re-locates its target via `indexOf` after the echo-cleanup sweep rather than trusting the original `idx`, since that sweep can shift array positions. BGM: `rubyStarBGM.mp3`. Sounds: `shield.mp3` (boss-tier death), `decoyDeath.mp3` (echo death), `bossQueenAttack1.mp3` + `gameModeClick.mp3` layered (Queen/echo attack), `ghostHunt2.mp3` (Queen phase-jump).
